...
export function SaaSWorkspace({ section }: { section: keyof typeof config }) {
  ...
  return <AppLayout title={meta.title} subtitle={meta.subtitle} actions={<button onClick={() => setShowModal(true)} className="inline-flex items-center gap-2 rounded-lg bg-brand-blue px-3 py-2 text-xs font-bold text-white shadow-glow-blue"><Plus className="h-4 w-4" /> {section === 'clients' ? 'Add client' : section === 'templates' ? 'New template' : 'Create sequence'}</button>}>
    <div className="space-y-6">
      ...
      {section === 'clients' && <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-gray-100 p-4"><span className="text-xs font-bold uppercase tracking-widest text-gray-400">{filteredClients.length} active clients</span><button onClick={() => action('Client CSV export prepared')} className="flex items-center gap-2 text-xs font-bold text-brand-blue"><Download className="h-4 w-4" /> Export</button></div><div className="divide-y divide-gray-100">{filteredClients.map((client) => <div key={client.email} className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-blue/10 font-bold text-brand-blue">{client.name.slice(0, 2)}</div><div><p className="font-bold text-gray-900">{client.name}</p><p className="text-xs text-gray-500">{client.email}</p></div></div><div className="flex items-center gap-6 text-sm"><div><p className="text-xs text-gray-400">Invoices</p><p className="font-bold text-gray-900">{client.invoices}</p></div><div><p className="text-xs text-gray-400">Outstanding</p><p className="font-bold text-gray-900">{client.outstanding}</p></div><StatusPill tone={client.status === 'At risk' ? 'amber' : 'green'}>{client.status}</StatusPill><Link href={`/app/clients/${encodeURIComponent(client.name.toLowerCase().replace(/[^a-z0-9]+/g, '-'))}`} onClick={() => action(`Opened ${client.name}`)} aria-label={`Open ${client.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-gray-50 hover:text-gray-900"><ArrowUpRight className="h-4 w-4" /></Link></div></div>)}</div></div>}
