-- Astrix AI - Supabase Database Schema
-- Complete backend setup with RLS policies
...
-- RLS Policies for reminder_logs table
...
-- Function to check if user can create new invoice (based on plan)
...
BEGIN
    ...
    -- Hook plan: max 3 invoices
    IF user_plan = 'hook' AND invoice_count >= 3 THEN
        RETURN FALSE;
    END IF;
    
    -- Solo and Agency: unlimited
    RETURN TRUE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
